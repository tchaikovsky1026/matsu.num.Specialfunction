...
final class NaiveBesselContinuedFraction {
    ...
    private static final double[] H_0_COEFFICIENT = {
            //H_0の漸近級数の連分数係数(50項)
    ...
    private static final double[] H_1_COEFFICIENT = {
            //H_1の漸近級数の連分数係数(50項)
    ...
    private NaiveBesselContinuedFraction() {
        //インスタンス化不可
        throw new AssertionError();
    }

    /**
     * H<sup>(2)</sup><sub>0</sub>(x)の漸近展開部分を連分数に変換したもの. <br>
     * t = i/(8x) を引数として, <br>
     * 1 + ((1^2)/(1))*t + ((1^2)*(3^2)/(1*2))*t^2 + ... <br>
     * を計算する仕組みである. <br>
     * 個の計算結果は, P<sub>0</sub>(x) + iQ<sub>0</sub>(x) となる.
     * 
     * @return 連分数
     */
    ...
    /**
     * 原理的, 直接的なH<sub>0</sub>の連分数.
     */
    private static ContinuedFractionFunction<RationalType, BigRationalElement> H0_inPrinciple() {
        final int kMax = 50;
    ...
    /**
     * 計算済みの係数からのH<sub>0</sub>の連分数.
     */
    private static DoubleContinuedFractionFunction H0_fromArray() {
        return DoubleContinuedFractionFunction.of(H_0_COEFFICIENT);
    }

    /**
     * 原理的, 直接的なH<sub>1</sub>の連分数.
     */
    private static ContinuedFractionFunction<RationalType, BigRationalElement> H1_inPrinciple() {
    ...
    /**
     * 計算済みの係数からのH<sub>1</sub>の連分数.
     * 
     */
    private static DoubleContinuedFractionFunction H1_fromArray() {
        return DoubleContinuedFractionFunction.of(H_1_COEFFICIENT);
    }
    ...
    private static final class H0Output {
        ...
        public static void main(String[] args) {
            ...
            System.out.printf("//H_0の漸近級数の連分数係数(%s項)_生値", coeffList.size());
            System.out.println();
     ...
     */
    @SuppressWarnings("unused")
    private static final class H1Output {

        public static void main(String[] args) {
            ContinuedFractionFunction<RationalType, BigRationalElement> fraction =
                    H1_inPrinciple();

            System.out.println("H_1 output");
            System.out.println();
            List<? extends Object> coeffList = fraction.coeffOfContinuedFraction();
            System.out.printf("//H_1の漸近級数の連分数係数(%s項)_生値", coeffList.size());
            System.out.println();
            for (Object c : coeffList) {
                System.out.println(c);
            }
