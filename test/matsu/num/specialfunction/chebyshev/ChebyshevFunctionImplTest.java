...
/**
 * {@link ChebyshevFunctionImpl} クラスのテスト.
 */
@RunWith(Enclosed.class)
final class ChebyshevFunctionImplTest {

    public static final Class<?> TEST_CLASS = ChebyshevFunctionImpl.class;

    public static class 生成に関する例外テスト {

        @Test(expected = IllegalArgumentException.class)
        public void test_負の次数はIAEx() {
            ChebyshevFunctionImpl.instanceOf(-1);
        }

        @Test(expected = IllegalArgumentException.class)
        public void test_大きすぎる次数はIAEx() {
            ChebyshevFunctionImpl.instanceOf(Integer.MAX_VALUE);
        }
    }
    ...
    public static class 次数10の正常値のパラメータ化テスト {

        private static final int DEGREE = 10;
        private final ChebyshevFunction function = ChebyshevFunctionImpl.instanceOf(DEGREE);
        ...
        public void test_chebyshevT(ChebyshevFixture fixture) {
            ...
            double result = function.chebyshevT(x);
        ...
        public void test_chebyshevU(ChebyshevFixture fixture) {
            ...
            double result = function.chebyshevU(x);

            assertThat(result, is(closeTo(expected, Math.abs(expected) * 1E-14)));
        }
    }

    public static class 異常値のテスト {

        private final ChebyshevFunction function = ChebyshevFunctionImpl.instanceOf(10);

        @Test
        public void test_chebyshevT_m1未満はNaN() {
            double x = Math.nextDown(-1d);

            assertThat(function.chebyshevT(x), is(Double.NaN));
        }

        @Test
        public void test_chebyshevT_1超過はNaN() {
            double x = Math.nextUp(1d);

            assertThat(function.chebyshevT(x), is(Double.NaN));
        }

        @Test
        public void test_chebyshevU_m1未満はNaN() {
            double x = Math.nextDown(-1d);

            assertThat(function.chebyshevU(x), is(Double.NaN));
        }

        @Test
        public void test_chebyshevU_1超過はNaN() {
            double x = Math.nextUp(1d);
