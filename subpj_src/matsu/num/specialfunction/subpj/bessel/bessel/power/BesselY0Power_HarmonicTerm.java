...
/**
 * <p>
 * Y0の調和数部分の計算の近似のためのターゲット. <br>
 * u = (x/2)^2 として, {@literal 0 <= u <= 1} を引数とする形で計算する. <br>
 * Y0 = -2/pi * (G(u)-(gamma+log(x/2))J0(x)) <br>
 * の形に分解したときの, <br>
 * Gの推定に関する. <br>
 * G(u) = Sum_{m=0 to inf} (-1)^m (H_m)u^m / (m!)^2 <br>
 * スケールは 1 とする.
 * </p>
 * 
 * @author Matsuura Y.
 */
final class BesselY0Power_HarmonicTerm extends RawCoeffCalculableFunction<DoubleDoubleFloatElement> {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Y0(x) について,");
        System.out.println("u = (x/2)^2, Y0(x) = -2/pi * (G(u)-(gamma+log(x/2))J0(x)) としたときの,");
        ...
        new EachApproxExecutor(order).execute(new BesselY0Power_HarmonicTerm());

        System.out.println("finished...");
    }

    private BesselY0Power_HarmonicTerm() {
        super();
    ...
    protected DoubleDoubleFloatElement calcValue(DoubleDoubleFloatElement u) {
        ...
        /*
         * G(u) = sum_{m=0 to inf} (-1)^m (H_m)u^m / (m!)^2
         */
